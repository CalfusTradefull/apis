...
describe('CustomersController', () => {
  ...
  describe('findAll', () => {
    it('should return a list of customers', async () => {
      ...
      jest.spyOn(customersService, 'findall').mockResolvedValue([mockCustomers] as  Customer[]);
   ...
   */
   describe('findOne', () => {
    ...
    it('should throw NotFoundException if customer is not found', async () => {
      ...
      await expect(controller.findOne('non-existent-id')).rejects.toThrowError(NotFoundException);
    });

    it('should throw BadRequestException with detailed error message for invalid UUID format', async () => {
      jest.spyOn(customersService, 'findOne').mockRejectedValue(new BadRequestException());
      await expect(controller.findOne('invalid-uuid')).rejects.toThrowError(BadRequestException);
    });

    it('should throw InternalServerErrorException for other unexpected errors', async () => {
      jest.spyOn(customersService, 'findOne').mockRejectedValue(new Error('Unexpected error'));
      await expect(controller.findOne('some-id')).rejects.toThrowError(InternalServerErrorException);
    });
  });
